<template>
  <div class="app">
    ...
    <header class="nav-bar">
      <div class="nav-title">{{ activeTab === 'food' ? '🥬 食材管理' : '🛒 购物清单' }}</div>
    ...
    <div class="storage-filter-row" v-if="activeTab === 'food' && activeCategory === 'all'">
      <div class="storage-filter-scroll">
        <button v-for="s in storageFilterOptions" :key="s.key"
          ...
          @click="activeStorage = s.key">{{ s.label }}</button>
      </div>
      ...
      <div v-for="group in groupedFoods" :key="group.category" class="food-group">
        ...
        <TransitionGroup name="food-card" tag="div" class="group-items">
          <div v-for="food in group.items" :key="food.id"
            :class="['food-card', {
              expired: food.daysLeft < 0, warning: food.daysLeft >= 0 && food.daysLeft <= 1,
    ...
    <div class="shop-list" v-if="activeTab === 'shop'">
      ...
      <div class="shop-section" v-if="uncheckedShopItems.length">
        ...
        <div v-for="item in uncheckedShopItems" :key="item.id"
          class="shop-item" @click="foodStore.toggleShopItem(item.id)">
          <div :class="['shop-check', { checked: item.checked }]">
      ...
      <div class="shop-section" v-if="checkedShopItems.length">
        ...
        <div v-for="item in checkedShopItems" :key="item.id"
          class="shop-item checked" @click="foodStore.toggleShopItem(item.id)">
          <div :class="['shop-check', { checked: item.checked }]">
            <span v-if="item.checked">✓</span>
          </div>
          <span class="shop-item-text">{{ item.text }}</span>
      ...
      <div class="empty-state" v-if="sortedShopList.length === 0">
        ...
        <div class="empty-hint">在上面输入要买的东西吧</div>
      </div>
    </div>
        ...
        <div class="modal-sheet">
          ...
          <div v-if="!editingFood && foodStore.templates.length" class="templates-section">
            ...
            <div class="templates-scroll">
              <div v-for="(tpl, i) in foodStore.templates.slice(0, 6)" :key="i"
                class="template-chip" @click="fillTemplate(tpl)">
                <span>{{ tpl.name }}</span>
          ...
          <div class="form-group">
            ...
            <div class="qty-row">
              <input v-model="form.quantity" :class="['form-input', 'qty-input', { 'form-input-error': errors.quantity }]"
                type="number" step="0.1" min="0.1" max="99.9" placeholder="0.1~99.9"
                @input="validateQty()" @blur="validateQty()" />
          ...
          <div class="form-group">
            ...
            <div class="date-dual-row">
              ...
              <div class="date-col days-col">
                <input v-model="form.days" :class="['form-input', { 'form-input-error': errors.days }]"
                  type="number" step="0.1" min="0.1" max="99.9" placeholder="天数"
                  @input="validateDayField(); recalcExpiry()" @blur="validateDayField(); recalcExpiry()" />
          ...
          <div class="form-group">
            ...
            <div class="category-picker">
              <button v-for="cat in foodCategories" :key="cat.key"
                :class="['category-option', { active: form.category === cat.key }]" @click="form.category = cat.key">
          ...
          <div class="form-group">
            ...
            <div class="storage-picker">
              <button v-for="s in storages" :key="s"
                :class="['storage-option', { active: form.storage === s }]" @click="form.storage = s">
                {{ storageIcon(s) }} {{ s }}</button>
            </div>
          </div>
...
const filterCategories = [
  ...
  { key: '乳制品', label: '乳制品', emoji: '🥛' }, { key: '调料', label: '调料', emoji: '🧂' },
...
const storageFilterOptions = [
  { key: 'all', label: '全部位置' }, { key: '冷藏', label: '❄️ 冷藏' },
  { key: '冷冻', label: '🧊 冷冻' }, { key: '常温', label: '🏠 常温' },
]
...
function switchTab(tab) {
  activeTab.value = tab
  if (tab === 'food') {
    batchMode.value = false; selectedIds.value = new Set()
  }
}
...
function toggleSelectAll() {
  ...
  else selectedIds.value = new Set(filteredFoods.value.map(f => f.id))
}
...
function handleCardClick(food) {
  if (batchMode.value) {
    ...
    if (next.has(food.id)) next.delete(food.id); else next.add(food.id)
...
function doBatchDelete() {
  ...
  selectedIds.value = new Set(); batchDeleteTarget.value = ''; batchMode.value = false
}

function batchMarkConsumed() {
  if (selectedIds.value.size) { foodStore.markConsumed([...selectedIds.value]); selectedIds.value = new Set() }
}
...
const wasteLevelClass = computed(() => {
  ...
  return r >= 30 ? 'waste-bad' : r >= 10 ? 'waste-warn' : 'waste-good'
})
...
function getDefaultForm() {
  return { name: '', quantity: 1.0, unit: '个', days: 7.0, category: '蔬菜', storage: '冷藏', purchaseDate: todayStr }
}
...
const isFormValid = computed(() =>
  form.value.name.trim() !== '' && errors.name === '' && errors.quantity === '' && errors.days === ''
)

function fillTemplate(tpl) {
  form.value.name = tpl.name; form.value.quantity = tpl.quantity; form.value.unit = tpl.unit
  form.value.category = tpl.category; form.value.storage = tpl.storage
...
function openAddModal() {
  editingFood.value = null; form.value = getDefaultForm()
  errors.name = ''; errors.quantity = ''; errors.days = ''
...
function saveFood() {
  ...
  if (editingFood.value) foodStore.updateFood(editingFood.value.id, data)
  else foodStore.addFood(data)
  closeModal()
}
...
function getCategoryEmoji(cat) { return filterCategories.find(c => c.key === cat)?.emoji || '📦' }
...
function requestNotification() {
  if ('Notification' in window && Notification.permission === 'default') {
    Notification.requestPermission().then(p => { if (p === 'granted') checkExpiryNotification() })
  }
}

function startNotificationTimer() {
  if (notificationTimer) clearInterval(notificationTimer)
  notificationTimer = setInterval(checkExpiryNotification, 1000 * 60 * 60 * 4)
}
